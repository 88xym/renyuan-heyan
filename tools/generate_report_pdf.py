...
def draw_page_header(canvas, doc) -> None:
    ...
    if HEADER_BANNER.exists():
        try:
            ...
            print(f"[提示] 页眉横幅图绘制失败: {HEADER_BANNER}", file=sys.stderr)
    else:
        print(f"[提示] 未找到页眉横幅图: {HEADER_BANNER}（请放置 表头照片.png）", file=sys.stderr)
    # 分隔线（正文区上方 1.5mm）
